...
var graph_width = 600, 
	...
	variable_panel_width = 291; // for mediation analysis interface
...
function preprocessVariable(data, variables) {
	for (var i = variables.length - 1; i >= 0; i--) {
		if (variables[i].type === "numerical") {
			...
			// hard-coded sorting order
			if (variables[i].name === 'Gender') {
				variables[i].labels = ['M', 'F'];
			}
		}
...
var task_configs = {
	...
	"admission(X-Y)": {
		// TODO: convert the hardcode path to Django static generated path
		"path": "/static/data/apply_data_X-Y.csv",
		"Variables": Apply3Variables,
		"preprocess": preprocessApply,
		"question": "",
		"next_link": '/presurvey',
		"object_name": 'Applicants'
	...
	"admission(X-MY)": {
		// TODO: convert the hardcode path to Django static generated path
		"path": "/static/data/apply_data_X-MY.csv",
		"Variables": Apply3Variables,
		"preprocess": preprocessApply,
		"question": "",
		"next_link": '/presurvey',
	...
	"admission(X-M-Y)": {
		// TODO: convert the hardcode path to Django static generated path
		"path": "/static/data/apply_data_X-M-Y.csv",
		"Variables": Apply3Variables,
		"preprocess": preprocessApply,
		"question": "",
		"next_link": '/presurvey',
...
function setTask(task_name) {
	...
	// update UI
	// see if an item's data-model value is equal to task_name
	var task_name_shown = $('.dropdown-task[data-model="' + task_name + '"]').html() || task_name;
	$("#dataset_name").html(task_name_shown);
...
function initializePageWithData() {
	...
	loadData(configs.data.path, function() {
		...
		if (CONDITION == 'control') {
			...
			d3.select("#chart_container").style("left", "294px");
			d3.select("#suggestion_container").style("left", "996px")
		}
		else {
			initGraph();	
		}
	...
	if (ASSISTING === false) {
		$('#main_vis_header').css('width', '700px');
		$('#related_vis_header').hide();
		$('#suggestion_container, #suggestion_container_cover').hide();
...
function run_regressions_based_on_variables(selected_vars) {
	...
	if (selected_vars.length == 2) {
		...
		clean_tested_relations();
		setImportance(1); // selected_vars.length == 1
	}
}
...
function clean_tested_relations() {
	...
	$(".trend-center, .trend-left, .trend-right, .trend-center-bottom")
	    .rotate(0)
	    .css({'opacity': 0, 'color': 'black'})
	    .removeClass('blink_me');
	$('#suggestion_container_cover').fadeIn();
	$('#chart_container .chart-top-div').css('opacity', 0);
}
...
function show_tested_relations() {
	...
	for (var x_name in visualized_relations.result) {
		if (x_name !== "const") {
			...
			main_chart = $('#chart1');
			if (x_name === 'goal' || x_name === 'department') {
				if (selected_vars.length == 2) {
			...
			else if (x_name === 'category' || x_name === 'gender') {
				if (selected_vars.length == 2) {
					// show the arrow in the center
					if (sig) {
						// we know it's descending trend for category vs. success
						if (y.name === 'Success') {
							$('.trend-center', main_chart).rotate(30).css({'opacity': 1, 'color': 'green'});	
						}
						// otherwise it's ascending for category vs. goal
						else {
							$('.trend-center', main_chart).rotate(-30).css({'opacity': 1, 'color': 'green'});		
						}
					}
					...
					if (sig) {
						// we know it's descending trend for goal
						$('.trend-center-bottom', main_chart).rotate(30).css({'opacity': 1, 'color': 'green'});
					}
					else {
	...
	$("#suggestion_container").html("");
	var tested_relations_processed = [];
	tested_relations.forEach(function(result, i) {
		var inconsistent = 0
		for (var variable_key in result) {
			if (variable_key === 'const' || !(variable_key in visualized_relations.result)) continue;
			console.log(result, visualized_relations.result);
			if (result[variable_key]['sig'] !== visualized_relations.result[variable_key]['sig'])  {
				inconsistent += 1;
				result['inconsistent_key'] = variable_key;
			}
			else {
				result['consistent_key'] = variable_key;
			}
			result['target_key'] = variable_key;
				
		}
		result['inconsistent'] = inconsistent;
	})
	// show inconsistent trends first then others, sort the relations by # of inconsistency
	tested_relations.sort((r1, r2 )=> -(r1['inconsistent'] - r2['inconsistent'])); 

	// show main chart importance	
	$('#chart_container .chart-top-div').css('opacity', 1);
	if (selected_vars.length === 1) {
		setImportance(1);	
	}
	else if (selected_vars.length === 3) {
		setImportance(3);	
		...
		var inconsistent_num = tested_relations.map(result => result.inconsistent).reduce((a, b) => a + b, 0);
		setImportance(3-inconsistent_num);
	}
	...
	tested_relations.forEach(function(result, i) {
		...
		var model_variables_array = model_variables_keys.map(key => Variables[nameToId(key)]).filter(elem => elem != undefined); // we have consistent key, etc.

		// generate the chart and append to the suggestion container
		// var block = $($("template#suggestion_block_template").html());
		var block = $($("template#suggestion_block_template_collapse").html());
		var block_id = "suggest-" + i;
		var collapse_id = "collapse-" + i;
		
		
		$('div.accordion-body', block).attr('id', collapse_id);
		$(".chart", block).attr("id", block_id);
		$("button.bring-to-main", block).click(function(){
			updateChartWithVariables(model_variables_array);
		})
		$('div.accordion-heading', block).click(function(){
			$('div.accordion-body').each(function(i) {
				if ( $(this).attr('id') === collapse_id ) {
					$(this).collapse('toggle');
					tested_relations[i]['expanded'] = !tested_relations[i]['expanded'];
					var expand_arrow_class = (tested_relations[i]['expanded'])? 'glyphicon-chevron-up' : 'glyphicon-chevron-down';
					$('.accordion-heading-expand span', this).attr('class', 'glyphicon ' + expand_arrow_class);
				}
				else {
					$(this).collapse('hide');
					tested_relations[i]['expanded'] = false;
					var expand_arrow_class = 'glyphicon-chevron-down';
					$('.accordion-heading-expand span', this).attr('class', 'glyphicon ' + expand_arrow_class);
				}
			})

			// blink the corresponding arrows on the main chart
			// find the target variable
			 blink_variable = '';
			for (var i = 0; i < tested_relations.length; i++) {
				if ( tested_relations[i]['expanded'] ) {
					blink_variable = tested_relations[i]['target_key'];
				}
			}
			// blink the target key
			if (blink_variable === '') {
				$('.trend-center, .trend-left, .trend-right, .trend-center-bottom', '#chart_container').removeClass('blink_me');
			}
			else {
				if (blink_variable === 'goal' || blink_variable === 'department') {
					if (selected_vars.length == 2) {
						$('.trend-center, .trend-left, .trend-right, .trend-center-bottom', '#chart_container').addClass('blink_me');
					}
					else if (selected_vars.length == 3) {
						$('.trend-center, .trend-left, .trend-right, .trend-center-bottom', '#chart_container').removeClass('blink_me');
						$('.trend-left, .trend-right', '#chart_container').addClass('blink_me');
					}
				}
				else if (blink_variable === 'category' || blink_variable === 'gender') {
					if (selected_vars.length == 2) {
						$('.trend-center, .trend-left, .trend-right, .trend-center-bottom', '#chart_container').addClass('blink_me');
					}
					else if (selected_vars.length == 3) {
						$('.trend-center, .trend-left, .trend-right, .trend-center-bottom', '#chart_container').removeClass('blink_me');
						$('.trend-center-bottom', '#chart_container').addClass('blink_me');
					}
				}
			}


			// console.log('toggle #' + collapse_id)
		...
		if (result['inconsistent']) {
			x_name = result['inconsistent_key'];
			visual_description = `Inconsistent trend between "${x.name}" and "${y.name}" detected (click here to view)`;
			$(".description", block).html(visual_description)
			                        .addClass('background-red');
			show_arrow_hardcoded(x_name, result[x_name]['sig'], block, model_variables_array.length, y);
		}
		else {
			x_name = result['consistent_key'];
			visual_description = `Consistent Trend between "${x.name}" and "${y.name}" Found (click here to view)`;
			$(".description", block).html(visual_description)
		...
		// for(var x_name in result) {
			...
			// 			visual_description = `Inconsistent Trend of "${x_name}" Detected`;
			// 		}
			// 		else {
			...
			// 				visual_description = `Consistent Trend of "${x_name}" Found`;
			// 			}
			// 		}
			// 	}
			// 	// $(".detail span.description", block).html(visual_description);
			// 	$(".description", block).html(visual_description)
			// 	                        .addClass(visual_inconsistent? 'background-red' : 'background-green');

			// 	// show trend arrows, HARDCODED, in the future we need to return coefficents from the regression results
			// 	main_chart = block;

			// 	// if there is one inconsistent key, only show the arrows for that key
	...
	setTimeout(function(){
		$('#suggestion_container_cover').fadeOut();
	}, 500); //show the panel after the delay
	


}

function show_arrow_hardcoded(x_name, sig, main_chart, model_variables_num, y) {
	if (x_name === 'goal' || x_name === 'department') {
		if (model_variables_num == 2) {
			// show the arrow in the center
			if (sig) {
				// we know it's descending trend for goal
				$('.trend-center', main_chart).rotate(30).css({'opacity': 1, 'color': 'green'});
		...
		if (model_variables_num == 2) {
			...
			if (sig) {
				...
				$('.trend-center-bottom', main_chart).css('opacity', 1);	
			}
		}
	}
}
...
function updateChartWithVariables(visualized_variables) {
	...
	// update chart
	selected_vars = d3.selectAll("circle.selected").data();
	var visual_setting = autoVisualSet(selected_vars);
	updateChartVisualSetting("chart1", visual_setting);	
...
function initGraph_Control() {
	...
	for (var i = 0; i < Variables.length - 1; i++) {
		for (var j = i + 1; j < Variables.length; j++) {
			drawline(Variables[i], Variables[j]);
		}
	}
}
...
function initChart() {
	...
	// top div
	charts.append('div')
	      .attr('class', 'chart-top-div')
	      .html('Chart Importance: <span class="star glyphicon glyphicon-star-empty" aria-hidden="true"></span>\
...
function setImportance(star_num) {
	$('#chart_container div.chart-top-div').css('opacity', 1);
	$('#chart_container div.chart-top-div span.star').each(function(i) {
		if (i < star_num) {
