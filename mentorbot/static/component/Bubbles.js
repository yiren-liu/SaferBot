...
function Bubbles(container, self, options) {
  ...
  this.typeInput = function (callbackFn) {
    ...
    inputText.setAttribute("placeholder", "Please type your response here.")
  ...
  // manually trigger bubble and inputCallbackFn
  this.input = function (text) {
    addBubble(
      '<span class="bubble-button bubble-pick">' + text + "</span>",
      function () { },
      "reply reply-freeform"
    );
    inputCallbackFn({
  ...
  // new reply function: directly input the text content onclick
  this.reply = function (turn) {
    iceBreaker = typeof turn === "undefined"
    turn = !iceBreaker ? turn : _convo.ice
    questionsHTML = ""
    ...
    if (turn.reply !== undefined) {
      ...
      // display clickables as buttons
      // for (var i = 0; i < turn.reply.length; i++) {
      //   ; (function (el, count) {
      //     questionsHTML +=
      //       '<span class="bubble-button" style="animation-delay: ' +
      //       animationTime / 2 * count +
      //       'ms" onClick="' + self + ".input('" + el + "', function(){});" +
      //       "\">" +
      //       el +
      //       "</span>"
      //   })(turn.reply[i], i)
      ...
      // put the first option in the text input area
      if (turn.reply[0] !== undefined) {
        s = document.querySelector('.input-wrap textarea');
        s.value = turn.reply[0];
      }
    }
    orderBubbles(turn.says, function () {
      bubbleTyping.classList.remove("imagine")
  ...
  var orderBubbles = function (q, callback) {
    ...
    //   setTimeout(resolve_fn, timeout);
    // });

    // var start = async function() {
    //   await setTimeoutPromise(animationTime);
    // }
  ...
  var addBubble = function (say, posted, reply, live) {
    ...
    if (reply !== "") {
      ...
      bubble.addEventListener("click", function (e) {
        if (e.target.classList.contains('bubble-button')) {
          ...
          // change to not display picked item
          // this.remove();
        }
      })
    }
    ...
    bubbleQueue = setTimeout(function () {
      ...
      // todo: if chat panel is minimized, override offsetWidth
...
function prepHTML(options) {
  ...
  appendCSS(relative_path + "component/styles/float.css")
}
