...
class EventArgumentModel:
    def __init__(self, model, device='cpu'):
        self.qa = QA_Model(model, device)
        self.slot_temp = [] # slot template
        self.slot_questions = [] # questions corresponding to slot template
    
    def update_template_dict(self, temp_dict):
        self.slot_temp = list(temp_dict.keys())
        self.slot_questions = list(temp_dict.values())

    def predict_slot(self, text):
        answers_dict = self.qa.answer(text, self.slot_questions, self.slot_temp)
        args = list(answers_dict.keys())
        
        # temporary solution, TODO: fix
        offsets_dict = dict(zip(args, [(0, 0)]*len(args)))
        return answers_dict, offsets_dict
